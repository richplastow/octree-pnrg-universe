 ...
 * - The children of an octant can be found using getChildOctantIds()
 * - In the special case of zero, the first child is a cyclical reference back
 *   to zero (it is its own parent)
 * @param {BigInt} octantId_u126 The octant ID (an integer, 0 to 2^126-1)
 * @returns {BigInt} The parent octant ID (an integer, 0 to 2^123-1
 */
export const getParentOctantId = (octantId_u126) => {
  validateU126(octantId_u126, 'getParentOctantId(): octantId_u126');

  // Divide by 8 to get the parent ID, shared by all its children.
  const parentId = octantId_u126 >> 3n;
